...
class SuggestionsView(discord.ui.View):
    def __init__(self, cog: "_CSClub_Suggestions"):
    ...
    def partial_message(self) -> Optional[discord.PartialMessage]:
        channel = self.cog.base.guild.get_channel(self.cog.SUGGESTIONS_CHANNEL_ID)
    ...
    async def create_suggestion(self, button: discord.ui.Button, interaction: discord.Interaction):
        async with self.edit_lock:
            ...
            # Add to database
            async with self.cog.bot.db.connect(writing=True) as conn:
                async with conn.cursor(transaction=True) as c:
                    ...
                    await c.execute(
                        'INSERT INTO csclub_suggestion VALUES (?, ?)',
...
def is_valid_suggestion_channel():
    async def predicate(ctx: commands.Context):
        ...
        cog = cast(_CSClub_Suggestions, ctx.bot.get_cog('_CSClub_Suggestions'))
...
class _CSClub_Suggestions(commands.Cog):
    SUGGESTIONS_CHANNEL_ID = 905955070934384671
    SUGGESTIONS_MESSAGE_ID = 907478104891621386

    def __init__(self, bot: TheGameBot, base: CSClub):
        self.bot = bot
        self.base = base
    ...
    async def on_suggestion_delete(self, thread: discord.Thread, *, keep_in_database=False):
        ...
        if not keep_in_database:
            ...
            async with self.bot.db.connect(writing=True) as conn:
                await conn.execute(
                    'DELETE FROM csclub_suggestion WHERE thread_id=?',
                    thread.id
                )
    ...
    async def fetch_suggestion_user_id(self, thread_id: int) -> int:
        async with self.bot.db.connect() as conn:
            async with conn.execute(
                    'SELECT user_id FROM csclub_suggestion '
    ...
    async def fetch_suggestion_user_ids(self, thread_ids: list[int]) -> dict[int, int]:
        ...
        async with self.bot.db.connect() as conn:
            async with conn.cursor() as c:
                query = """
                SELECT thread_id, user_id FROM csclub_suggestion
                WHERE user_id IN ({})
    ...
    @app_commands.guilds(CSClub.GUILD_ID)
    @app_commands.describe(
        title='The new title for your suggestion (at most 50 letters).'
    )
    @app_commands.checks.cooldown(2, 600, key=lambda inter: inter.channel_id)
    @is_valid_suggestion_channel()
    async def client_rename_suggestion(self, inter: discord.Interaction, title: str):
