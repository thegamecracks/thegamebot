...
class _SignalHill_Status(commands.Cog):
    ...
    def __init__(self, bot: TheGameBot, base: SignalHill):
        ...
        self.SERVER_STATUS_GRAPH_DEST = settings.get('signal_hill', 'status_cdn')
    ...
    @commands.is_owner()
    async def client_server_status(self, ctx: Context):
        """Manage the Signal Hill server statuses."""

    @client_server_status.command(name='create')
    async def client_server_status_create(
    ...
    async def client_server_status_toggle(self, ctx: Context):
