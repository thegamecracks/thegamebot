...
from bot import checks, converters
from main import Context, TheGameBot


class GiveawayContext(Context):
    last_giveaway: discord.Message
...
def check_giveaway_status(
    ...
    """
    e = commands.BadArgument(bad_response)

    async def predicate(ctx: Context):
        channel_id = ctx.bot.get_settings().get('signal_hill', 'giveaway_channel_id')
        channel = ctx.bot.get_channel(channel_id)

        ctx.last_giveaway = m = await discord.utils.find(
...
class _SignalHill_Giveaway(commands.Cog):
    ...
    def __init__(self, bot: TheGameBot, base: SignalHill):
        ...
        self.GIVEAWAY_CHANNEL_ID = settings.get('signal_hill', 'giveaway_channel_id')
        self.GIVEAWAY_EMOJI_ID = settings.get('signal_hill', 'giveaway_emoji_id')
    ...
    async def client_giveaway(self, ctx: Context):
    ...
    async def _giveaway_get_message(self):
        ...
        try:
            return await anext(self.giveaway_channel.history(limit=1))
    ...
    async def _giveaway_get_participants(
        self, message: discord.Message, *, ignored=None
    ):
        if not ignored:
    ...
    @is_giveaway_running()
    async def client_giveaway_cancel(self, ctx: GiveawayContext, *, description: str = None):
...
The giveaway message is deleted if no description is given."""
    ...
    @is_giveaway_running()
    async def client_giveaway_edit(self, ctx: GiveawayContext, *, flags: GiveawayEditFlags):
...
description: The description of the giveaway."""
    ...
    @is_giveaway_running()
    async def client_giveaway_finish(self, ctx: GiveawayContext, ping: bool = True):
    ...
    async def client_giveaway_reroll(
            self, ctx: GiveawayContext, winner: Optional[discord.User], *, reason):
...
reason: The reason for rerolling the winner."""
    ...
    async def client_giveaway_simulate(self, ctx: Context, *, flags: GiveawayStartFlags):
...
description: The description of the giveaway."""
    ...
    async def client_giveaway_start(self, ctx: Context, *, flags: GiveawayStartFlags):
